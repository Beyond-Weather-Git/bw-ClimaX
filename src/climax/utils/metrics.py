...
def rmse(pred, y, vars, lat=None, mask=None):
    loss_dict = {}
    loss_dict["loss"] = torch.sqrt(((pred - y) ** 2).mean())
    return loss_dict


def lat_weighted_mse_val(pred, y, transform, vars, lat, log_postfix):
    """Latitude weighted mean squared error
    Args:
        y: [B, V, H, W]
        pred: [B, V, H, W]
    ...
    with torch.no_grad():
        for i, var in enumerate(vars):
            loss_dict[f"w_mse_{var}_{log_postfix}"] = (
                error[:, i] * w_lat
            ).mean()

    loss_dict["w_mse"] = np.mean(
        [loss_dict[k].cpu() for k in loss_dict.keys()]
    )
    ...
    with torch.no_grad():
        for i, var in enumerate(vars):
            ...
            loss_dict[f"acc_{var}_{log_postfix}"] = torch.sum(
                ...
                torch.sum(w_lat * pred_prime**2)
                * torch.sum(w_lat * y_prime**2)
            )
...
def lat_weighted_nrmses(pred, y, transform, vars, lat, clim, log_postfix):
    ...
    with torch.no_grad():
        for i, var in enumerate(vars):
            ...
            y_ = y[:, i]  # B, H, W
            error = (
                torch.mean(pred_, dim=0) - torch.mean(y_, dim=0)
            ) ** 2  # H, W
...
def lat_weighted_nrmse(pred, y, transform, vars, lat, clim, log_postfix):
    ...
    nrmses = lat_weighted_nrmses(
        pred, y, transform, vars, lat, clim, log_postfix
    )
    nrmseg = lat_weighted_nrmseg(
        pred, y, transform, vars, lat, clim, log_postfix
    )
    loss_dict = {}
    for var in vars:
...
def lat_weighted_mean_bias(
    pred, y, transform, vars, lat, log_steps, log_days, clim
):
    """
    y: [N, T, 3, H, W]
    pred: [N, T, 3, H, W]
